...
private:
  ...
  void dronesPoseCallback(const sensor_msgs::PointCloud2ConstPtr& pose);

  void resetBuffer_d();
  void resetBuffer_d(Eigen::Vector3d min_pos, Eigen::Vector3d max_pos);
  void clearAndInflateLocalMap_d();
  ...
  ros::Subscriber indep_cloud_sub_, indep_odom_sub_, extrinsic_sub_, drones_pose_sub;
...
inline int GridMap::getInflateOccupancy(Eigen::Vector3d pos) {
  ...
  return int(md_.occupancy_buffer_inflate_[toAddress(id)] || md_drones.occupancy_buffer_inflate_[toAddress(id)]);
}
